(function () {
    ...
    function AnrLayoutCtrl($scope, toastr, $http, $q, $mdMedia, $mdDialog, $timeout, gettextCatalog, TableHelperService, ModelService,
        ...
        $scope.queryRecSearch = function (query) {
            ...
            var q = $q.defer();
            RecommandationService.getRecommandations({anr: $scope.model.anr.id, filter: query}).then(function (data) {
        ...
        $scope.detachRecommandation = function (ev, recommandation) {
            ...
            $mdDialog.show(confirm).then(function() {
                var RecommandationService = $injector.get('ClientRecommandationService');
                RecommandationService.detachFromRisk($scope.model.anr.id, recommandation.id,
                    function () {
                        updateRecommandations();
        ...
        var updateRecommandations = function () {
            var RecommandationService = $injector.get('ClientRecommandationService');
            RecommandationService.getRiskRecommandations($scope.model.anr.id, $scope.sheet_risk.id, false).then(function (data) {
